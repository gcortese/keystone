...
exports = module.exports = function(req, res) {
	...
	{
		itemQuery = req.list.model.findById(req.params.item).where(req.list.get('owner'), req.user).select();
	}
	else 
	{
		itemQuery = req.list.model.findById(req.params.item).select();
	}
	//FABRIZIO
	...
	itemQuery.exec(function(err, item) {
		...
		var renderView = function() {
			...
			async.each(relationships, function(rel, done) {
				...
				keystone.render(req, res, customView, {
					section: section,
