...
function render(req, res, view, ext) {
	...
	//FABRIZIO
	var templatePath;
	if (req.user && !req.user.isAdmin && (view == 'list' || view == 'item' || view == 'itemCustom'))
	{
		//templatePath = __dirname + '/../../../../templates/views/iframe-' + view + '.jade';//in front-end page
		templatePath = __dirname + '/../../templates/views/iframe-' + view + '.jade';//custom page in keystone (no nav header,...)

		if (view == 'itemCustom')
		{
			templatePath = __dirname + '/../../templates/views/item-custom.jade';//custom page in keystone (no navheader, react view modificata)
		}
		console.log('rendering -----------------------' + templatePath);
	}
	else {
		templatePath = __dirname + '/../../templates/views/' + view + '.jade';
	}
	//FABRIZIO
	...
	var locals = {
		...
		UserListName: keystone.get('user model'),
		user: req.user,
		title: 'Keystone',
		...
		navLinks : keystone.get("siteNavBar"),//FABRIZIO
		subHeaderLinks: keystone.get("dashboardSubHeader")//FABRIZIO
	};
