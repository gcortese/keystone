	...
	var initList = function(protect, isItem) {
		return function(req, res, next) {
			...
			if (!req.list || 
				...
				( protect && req.params.item && !req.user.isAdmin && req.list.key == keystone.get('user model') && !keystone.get('mongoose').Types.ObjectId(req.params.item).equals( req.user._id ) )) //-----------FABRIZIO//v2.0 se liste aperte/condivise...come limitare accesso a lista training session a teacher? xes 
			{
