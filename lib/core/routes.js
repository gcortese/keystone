...
function routes(app) {
	...
	var initList = function(protect, isItem) {
		return function(req, res, next) {
			req.list = keystone.list(req.params.list);

			if(keystone.get('mongoose').Types.ObjectId(req.params.item).equals( req.user._id )){
				console.log("self user");
			}
			
	//v1.0 non passa middleware se:
	//- (la lista ha ruolo proprietario e utente loggato ha ruolo diverso da quello proprietario della lista)
	//- se voglio item/form-dettaglio e lista non ha ruolo proprietario e non sto guardando me stesso...valido x esporre user me stesso a me stesso soltanto
	//- (se voglio lista e questa non ha ruolo proprietario)
	//passa sempre: - se admin
			//if (!req.list || (protect && req.list.get('hidden'))) {
			//if (!req.list || (protect && req.list.get('hidden')) || ( protect && !req.user.isAdmin && req.list.get('owner') && req.user.role != req.list.get('owner') ) || ( protect && req.params.item && !req.user.isAdmin && !req.list.get('owner') && ! keystone.get('mongoose').Types.ObjectId(req.params.item).equals( req.user._id ) || ( protect && !req.params.item && !req.list.get('owner') && !req.user.isAdmin) )) //-----------FABRIZIO
	//-----------FABRIZIO//v2.0 se liste aperte/condivise...pb per liste senza owner, per cui utente che sa link vede tutti gli item della lista
//			if (!req.list || 
//				(protect && req.list.get('hidden')) || 
//				( protect && req.params.item && !req.user.isAdmin && req.list.key == keystone.get('user model') && !keystone.get('mongoose').Types.ObjectId(req.params.item).equals( req.user._id ) )) 
			
			//v3.0 profilo utente modificato via form custom, tolto permesso a users/:id; liste visibili solo a utente non admin solo se lista ha campo owner e filtrate per owner
			if (!req.list || 
				(protect && req.list.get('hidden')) ||
				(protect && !req.user.isAdmin && !req.list.get('owner'))	)	
			{
