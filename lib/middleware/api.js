 ...
 *   * `res.apiBadRequest(err, msg)` 
 *   * `res.apiForbidden(err, msg)`
 *   * `res.apiNotFound(err, msg)`
 *   * `res.apiNotAllowed(err, msg)`
 *   * `res.apiMethodNotAllowed(err, msg)`
 *
 * ####Example:
 *
...
exports = module.exports = function(keystone) {
	return function initAPI(req, res, next) {
		...
		res.apiBadRequest = function (err, msg) {
			res.apiError('invalid request', err || undefined, msg || 'bad request', 400);
		};

		res.apiForbidden = function (err, msg) {
			res.apiError('access not allowed', err, msg || 'forbidden', 403);
		};
		...
		res.apiMethodNotAllowed = function(err, msg) {
			res.apiError('wrong verb', err || undefined, msg || 'method not allowed', 405);
		};
